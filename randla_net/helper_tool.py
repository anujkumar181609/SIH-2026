...
try:
    cpp_subsampling = importlib.import_module('cpp_wrappers.cpp_subsampling.grid_subsampling')
    if not hasattr(cpp_subsampling, 'compute'):
        raise ImportError("Compiled extension found as an empty namespace package (no 'compute' attribute) — "
                           "likely a Linux .so on a non-Linux platform.")
except ImportError:
    sys.path.append(os.path.join(BASE_DIR, 'utils', 'cpp_wrappers', 'cpp_subsampling'))
    cpp_subsampling = importlib.import_module('grid_subsampling_cpu')
    print("[helper_tool] Using pure-Python CPU grid_subsampling fallback (no compiled extension found).")
try:
    nearest_neighbors = importlib.import_module('nearest_neighbors.lib.python.nearest_neighbors')
except ImportError:
    sys.path.append(os.path.join(BASE_DIR, 'utils', 'nearest_neighbors'))
    nearest_neighbors = importlib.import_module('knn_cpu')
    print("[helper_tool] Using pure-Python CPU nearest_neighbors fallback (no compiled extension found).")
